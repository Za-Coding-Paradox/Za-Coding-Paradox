```md
# Abdullah Ahmad Khan

Computer Science undergraduate interested in **systems programming, low-level engineering, algorithms, and AI**.

## Who I Am

I like building software from first principles and understanding how things work under the hood.

## What I Do

I work mainly with **C++, Rust, Python, and Linux**, building systems software, developer tools, algorithms, and AI-focused projects.

## What I'm Studying

Currently studying **Computer Science**.

## Interests

- Systems Programming
- Low-Level Software
- Algorithms & Data Structures
- Artificial Intelligence
- Developer Tools
- Runtime & Memory Architecture

## Specialties

`C++` · `Rust` · `Python` · `Javascript` · `Typescript` · `Reach` `NextJS` · `GraphQL` · `Linux` · `Git` · `Docker`

## Relevant Projects

### [Custom Memory Allocator](https://github.com/Za-Coding-Paradox/Custom-Memory-Allocator)
A custom memory allocator focused on low-level memory management.

### [AI Urban Delivery System](https://github.com/Za-Coding-Paradox/AI-Urban-Delivery-System)
An AI-focused system for urban delivery and routing.

### [Shinigami](https://github.com/Za-Coding-Paradox/Shinigami)
Software experiments and ongoing development work.

## Links

[GitHub](https://github.com/Za-Coding-Paradox) · [LinkedIn](https://www.linkedin.com/in/abdullah-ahmad-khan-a22a17414)

## Metrics

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Za-Coding-Paradox&show_icons=true&hide_border=true&rank_icon=github)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Za-Coding-Paradox&layout=compact&hide_border=true)
```
